...
/*   Updated: 2025/02/26 17:51:40 by mfahmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_image(t_info *all)
{
	...
	if (!all->image->img_floor || !all->image->img_wall
		|| !all->image->img_player_front || !all->image->img_player_left
		|| !all->image->img_player_right || !all->image->img_player_back
		|| !all->image->img_exit_close || !all->image->img_coin)
		free_all(all);
}
	...
	while (all->map[i])
	{
		...
		while (all->map[i][j])
		{
			if (all->map[i][j] == '0')
				...
				mlx_put_image_to_window(all->mlx, all->mlx_new_win,
					all->image->img_exit_close, j * IMG_WIDTH, i * IMG_HEIGHT);
			else if (all->map[i][j] == 'P')
				add_player_m(all, i, j);
			j++;
		}
		i++;
	}
